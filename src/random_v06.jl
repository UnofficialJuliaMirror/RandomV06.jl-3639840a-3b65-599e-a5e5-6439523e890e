...
This file is a slight modification of the following file from Julia:
https://github.com/JuliaLang/julia/blob/v0.6.4/base/random.jl

The license for Julia is  MIT: https://julialang.org/license
=#
...
randjump(r::MersenneTwister, jumps::Integer) = randjump(r, jumps, Random.DSFMT.JPOLY1e21)
=#
...
@inline rand(r::MersenneTwister, ::Type{T}) where {T<:Union{Bool, Int8, UInt8, Int16, UInt16, Int32, UInt32}} = rand_ui52_raw(r) % T
...
rand(r::AbstractRNG, T::Type, dims::Dims) = rand!(r, Array{T}(undef, dims))
...
function rand_AbstractArray_Float64!(r::MersenneTwister, A::AbstractArray{Float64}, n=length(A), ::Type{I}=CloseOpen) where {I<:FloatInterval}
...
function rand!(r::MersenneTwister, A::Array{UInt128}, n::Int=length(A))
    ...
    if n > 0
        u = rand_ui2x52_raw(r)
        for i = 1:n
            @inbounds A[i] ⊻= u << (12*i) # DAS
        end
    end
    A
end

function rand!(r::MersenneTwister, A::Array{T}) where {T<:Union{Int8, UInt8, Int16, UInt16, Int32, UInt32, Int64, UInt64, Int128}}
    n=length(A)
    n128 = n * sizeof(T) ÷ 16
...
if Base.GMP.VERSION.major >= 6
    ...
    function rand(rng::AbstractRNG, g::RangeGeneratorBigInt)
        ...
        while true
            ...
            xd = ccall((:__gmpz_limbs_write, :libgmp), Ptr{Limb}, (Ptr{BigInt}, Clong), Ref(x), g.nlimbs)
            limbs = unsafe_wrap(Array, xd, g.nlimbs)
            rand!(rng, limbs)
            limbs[end] &= g.mask
            ccall((:__gmpz_limbs_finish, :libgmp), Nothing, (Ptr{BigInt}, Clong), Ref(x), g.nlimbs)
            x <= g.m && break
        end
        ccall((:__gmpz_add, :libgmp), Nothing, (Ptr{BigInt}, Ptr{BigInt}, Ptr{BigInt}), Ref(x), Ref(x), Ref(g.a))
        return x
    end
else
    function rand(rng::AbstractRNG, g::RangeGeneratorBigInt)
        ...
        while true
            rand!(rng, g.limbs)
            g.limbs[end] &= g.mask
            ccall((:__gmpz_import, :libgmp), Nothing,
                  (Ptr{BigInt}, Csize_t, Cint, Csize_t, Cint, Csize_t, Ptr{Limb}),
                  Ref(x), length(g.limbs), -1, sizeof(Limb), 0, 0, g.limbs)
            x <= g.m && break
        end
        ccall((:__gmpz_add, :libgmp), Nothing, (Ptr{BigInt}, Ptr{BigInt}, Ptr{BigInt}), Ref(x), Ref(x), Ref(g.a))
        return x
...
rand(rng::AbstractRNG, r::AbstractArray{T}, dims::Dims) where T = rand!(rng, Array{T}(undef, dims), r) 
...
bitrand(r::AbstractRNG, dims::Int...) = rand!(r, BitArray(undef, dims))

bitrand(dims::Dims)   = rand!(BitArray(undef, dims))
bitrand(dims::Int...) = rand!(BitArray(undef, dims))
